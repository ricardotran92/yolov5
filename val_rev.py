...
def run(
            ...
            if nl:
                ...
                for class_i in range(nc):  # loop over each class
                    ...
                    if class_indices.any():  # check if there are any labels of this class
                        ...
                        ious_img_padded = np.pad(ious_img, (0, len(class_indices) - len(ious_img)), 'constant', constant_values=0)
    ...
    # pf = "%22s" + "%11i" * 2 + "%11.3g" * 5  # new: print format
    # LOGGER.info(pf % ("all", seen, nt.sum(), mp, mr, map50, map, avg_ious))  # new: add avg_ious to log
    if nt.sum() == 0:
